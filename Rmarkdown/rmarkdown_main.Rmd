...
* ~2hrs: Introduction and practice      

* ~15min: pause  

* ~1min: other formats (*.docx*, *.pdf*)  
...
* Plain text files that typically have the file extension *.md*.    

* Example: https://github.com/seb951/rmarkdown_workshop/blob/master/README.md    
(look at github render and raw text)
...
* https://rmarkdown.rstudio.com/ (Most of these packages are maintained by the Rstudio team - Yihui Xie)    

* Plain text files that typically have the file extension *.Rmd*.  

* The R package `rmarkdown` package feeds the *.Rmd* file to `knitr`.   
...
* "\&nbsp;  " character (html Non-Breaking SPace) for extra line spacing is usefull.  

* A table (more on this later):
...
* Look at html output. What happened?   

* Look at the *.Rmd* file structure  

#Exercice 1
* Delete all of the R code chunks and write a some Markdown (some sections, some italicized text, and an itemized list). Convert the document to a webpage.  


#Header (YAML Ain't Markup Language)
`---`  
`title: "Rmarkdown"`  
...
* Below, I use five arguments. Take a look at them in the *.Rmd* file.  

* Arguments can be redefined later for other code chunks  
...
* You can also run inline code using the using the \`   \` symbols and specifying the programming language `r`: 10+5=`r 10+5`

#Exercice 3
* Add a code chunks that will:
    + load an R package
    + create a plot
    + create some output of a function
* Run inline code
    
#R markdown can read and execute different languages!
* Click green arrow in *.Rmd* file to execute code in console.  
**bash**
```{bash}
ls -1
```
...
* Mathematical material is set off by the use of single dollar-sign characters.
...
* See [these](http://www.math.mcgill.ca/yyang/regression/RMarkdown/example.html) example equations from an McGill math Rmarkdown tutorial.  
...
* Images can interpreted directly by `knitr` as below:  

```{r graphic_example, out.width = "20%", fig.cap = "A random orange man", echo = T}
...
#Exercice 4
* Add a picture from the web using both `knitr` and `pandoc`  
...
#Exercice 5
* Create a library (".bib") with the references for your five favorite scientific papers.

* Use another Citation Style Language (e.g. Nature, Science, PLOS ONE, etc.). 

...
* Preferably Rstudio 1.2 (https://www.rstudio.com/products/rstudio/download/preview/). Note that this version is not officially released yet. If you don't have it / don't want to install it, not big deal, you'll still be able to do 98% of the workshop.
...
````chunks  
`r ''````{r code chunk example, include = T, message = T, warning=T, echo = T, fig.cap="Figure 1"}  
#Running some R code.
x = rexp(1000)  
min(x)  
max(x)  
...
````global  
`r ''````{r roaches, out.width = "25%",fig.cap = "Cockroaches man", echo = F,out.extra='style="float:right; padding:10px"'}
knitr::include_graphics("https://en.wikipedia.org/wiki/File:Snodgrass_common_household_roaches.png")  
```
````

```{r roaches, out.width = "25%",echo = F,out.extra='style="float:right; padding:10px"'}
...
* Find and use another Citation Style Language (e.g. Nature, PLOS ONE, Indian Journal Of Dermatology, etc.). (hint: type 't' in GitHub repo to activate search). 

# Cheatsheets and help
* [Lessons](https://rmarkdown.rstudio.com/lesson-1.html)  

* [Book](https://bookdown.org/yihui/rmarkdown/)   
  * This is a great reference. In addition, it is (of course) written in `R mardown` and publicly available on [GitHub]( https://github.com/rstudio/rmarkdown-book). As such, if you see something you like, go see how it was written in the *.Rmd* version!  

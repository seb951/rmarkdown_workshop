...
• ~1h30min: intro + html format  
• ~10min. pause  
• ~1hr: other formats (.docx, .pdf, shiny app)  
...
• It is subsequently tranformed into `html`/`.tex`/`.docx` by `pandoc`. Note that `.tex` files need to be transformed by `pdflatex` into `.pdf` files. We'll come back to that later.  
• `rmardown`, `knitr`, and ``pandoc` are built in `Rstudio`. `pdflatex` is not.  
...
• It uses Python-style indentation to specify options  
...
• `word_document`   
• interactive shiny apps (We may get to that later)  
...
• In this example, the first code chunk specifies options. These can be redefined later. 
...
• Here I just googled and download the .csl file for PeerJ Most journals should be on this [github repo](https://github.com/citation-style-language/styles). But you could create your own (*Yet Another Citation Format...*)  
• A `.bib` file will contains the bibliographic information of our document. Here: a `.bib` file was created in [*Papers3 (reference management software)* (https://www.readcube.com/papers/mac) with some references (eg. @Yoshida:2014bn, @ThibertPlante:2010vw, @Wagner:2012hw).  
...
&nbsp;  
• In both these cases, graphs are rendered with `pandoc` and not `knitr`, so pandoc options are given, not R graphing options. It's simple, but options are tricky.  

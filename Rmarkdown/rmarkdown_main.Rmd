...
• Follow the .html document + in Rstudio, look at the .Rmd version and experiment.  
...
• Note that several options can be specified either for the whole document (in the YAML header), or the code chunks, or both (chunks options supersede YAML header).  
...
• `pdf_document` - This will require that you have a [Latex software](https://www.latex-project.org/get/) installed - (We may get to that later).    
...
• Also, try clicking the green arrow on the right to execute code.   
...
• The use of double dollars quote allows for displayed formulas. $$X_i$$  
...
• Otherwise, you may specify a bibliography in the YAML metadata header.  
...
```{r view, out.width = "20%", fig.cap = "A random orange man", echo = FALSE}

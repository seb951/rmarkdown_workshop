...
* ~15 minutes: shiny. [Workshop here](https://github.com/seb951/rmarkdown_workshop/blob/master/Rmarkdown/shiny.Rmd)     
...
* How does it work: 
    ...
    + [`Pandoc`](https://pandoc.org/) is an universal document converter, not an R package.
...
* Markdown provides an easy way to make standard types of formatted text, like:    
    ...
    + \# at the beginning of line for a header level (6 levels, \# to \#\#\#\#\#\#)
...
* By default `R studio` creates a *Global Options* code chunk (remember that `knitr` executes code and creates a new markdown (*.md*) document which includes the code and output). Let's examine this chunk:    
...
`![optinal caption here](https://static.independent.co.uk/s3fs-public/styles/article_large/public/thumbnails/image/2017/09/12/11/naturo-monkey-selfie.jpg){width=250px}`  
...
* If this figure is small, it can be added to the text directly: eg.: Today, we are using  ![](../figures/rstudio.png){width=40px} to generate webpages with ![](https://static.independent.co.uk/s3fs-public/styles/article_large/public/thumbnails/image/2017/09/12/11/naturo-monkey-selfie.jpg){width=30px} images...
...
* Footnotes are easy when you have a few references. Use `[^1]` and add reference at the end [^1].    

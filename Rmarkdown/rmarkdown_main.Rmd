...
# Introduction  
* You can download the workshop material on [GitHub](https://github.com/seb951/rmarkdown_workshop) ![](../figures/octocat.png){width=40px}. 

* Don't hesitate to comment (new issues), or request changes (pull request) on GitHub ![](../figures/octocat.png){width=40px}.   
...
* ~1 hour: other formats (*.docx*, *.pdf*, and others). [Workshop here](https://seb951.github.io/rmarkdown_workshop/Rmarkdown/rmarkdown_word_pdf.html)    
...
* The [cheatsheet v1](https://www.rstudio.com/wp-content/uploads/2015/02/rmarkdown-cheatsheet.pdf) or [v2](https://github.com/rstudio/cheatsheets/raw/master/rmarkdown-2.0.pdf) is your friend.  
...
````chunks  
`r ''````{r example, include = T, message = T, warning=T, echo = F, fig.cap="Figure 1"}  
#Running some R code.
x = rexp(1000)  
min(x)  
max(x)  
...
If this figure is small, it can be added to the text directly: eg.: Today, we are using  ![](../figures/rstudio.png){width=44px} to generate webpages with ![](https://upload.wikimedia.org/wikipedia/commons/7/7f/Rosa_persica%3B_Baikonur_01.jpg){width=30px} images...
...
````global  
`r ''````{r graphic_example, out.width = "20%", fig.cap = "An orange man", echo = F,fig.align = "center"}  
...
````global  
`r ''````{r cockroaches, out.width = "25%",fig.cap = "Cockroaches man", echo = F,out.extra='style="float:right; padding:10px"'}
knitr::include_graphics("https://en.wikipedia.org/wiki/File:Snodgrass_common_household_roaches.png")  
```
````

```{r cockroaches, out.width = "25%",echo = F,out.extra='style="float:right; padding:10px"'}
knitr::include_graphics("https://upload.wikimedia.org/wikipedia/commons/7/72/Snodgrass_common_household_roaches.png")
```

#### Cockroaches {.unnumbered}
...
The cockroaches are an ancient group, dating back at least as far as the Carboniferous period...   
&nbsp;  
&nbsp;  
&nbsp;  
&nbsp;  
...
### 2 figures in 2 columns {.unnumbered}

````2figs  
`r ''````{r out.width=c('50%', '50%'), fig.show='hold',echo=F}
boxplot(1:10)
plot(rnorm(10))
```
````
...
* But now, I often copy *.bib* references directly from Google Scholar and add it to a *.bib* database text file.   
...
## Exercice 6
* Create a *.bib* database text file with the references for your five favorite scientific papers.

* Find and use another Citation Style Language (e.g. Nature, PLOS ONE, Indian Journal Of Dermatology, etc.). (hint: type 't' in GitHub repo linked in Section 10 to activate search function). 

# Cheatsheets and help
* [Lessons](https://rmarkdown.rstudio.com/lesson-1.html)  

* [Book](https://bookdown.org/yihui/rmarkdown/)   
  * This is a great reference. In addition, it is (of course) written in `R mardown` and publicly available on [GitHub]( https://github.com/rstudio/rmarkdown-book). As such, if you see something you want to reproduce, go see how it was written in the *.Rmd* version!  

...
* Look at this [example](https://gallery.shinyapps.io/087-crandash/) or [that one ](https://gallery.shinyapps.io/lake_erie_fisheries_stock_assessment_app/). See [gallery](https://shiny.rstudio.com/gallery/) for more examples.  

* To learn more, look at official ![](rsconnect/documents/rstudio.png){width=44px} documentation on [Interactive Documents](http://rmarkdown.rstudio.com/authoring_shiny.html).
...
* The code below demonstrates how a standard plot can be made interactive by wrapping it in the Shiny `renderPlot` function (*output*).   
  
* The `selectInput` and `sliderInput` functions create the *input* widgets used to generate plots.    


```{r eruptions, echo=T}
#Faithful geyser dataset to generate histogram and density function.
knitr::kable(head(faithful),digits =1,caption = "Ye old faithful", align = "c") 
...
* Also need to specify remote account info
  ...
  + Then run function below with personnal info.  
    
```rsconnect
rsconnect::setAccountInfo(name='yourname',token='alphanumeric',secret='ASCII') 
```

* Publish document in Rtudio by clicking on *blue eye* ![](rsconnect/documents/blue_eye.png){width=120px} at the top right corner. Voilà!  

# Set-up your own shiny app
